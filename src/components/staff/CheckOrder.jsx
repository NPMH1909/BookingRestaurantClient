...
import { useGetRestaurantByStaffQuery } from "../../apis/userApi";
...
const Checkin = () => {
  ...
  const {
    ...
    error: orderError,
    refetch
  } = useGetCompletedOrdersQuery(restaurantId);
  const [updateCheckInOrder, { isLoading: isAdded, isError: isAddError }] =
    useUpdateOrderStatusMutation();
  const selectedId = useSelector((state) => state.selectedId.value);
  const handleUpdateCheckIn = async () => {
    try {
      const message = await updateCheckInOrder({ id: selectedId, status: 'ONHOLD' }).unwrap();
      refetch()
      return message;
    } catch (error) {
      Toast.fire({
  ...
  return (
    <>
      <AdminLayout
        ...
        noDelete={true}
        noUpdate={true}
        headerUpdate="Xác nhận checkin cho đơn hàng"
        bodyUpdate={
          <Card>
            <CardBody>
