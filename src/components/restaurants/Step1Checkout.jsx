    ...
    try {
      const message = await createOrder({
        ...
        menuItems: menu.map((item) => ({
          ...
          priceAtBooking: item.price * (1 - (item.promotion?.discountPercent || 0) / 100),
          name: item.name,
          unit: item.unit,
      ...
      if (message.checkoutUrl) {
        window.location.replace(message.checkoutUrl);
      }
      else {
        const orderId = message?.order?._id;
        console.log('orderId', orderId)
        navigate(
          `/checkout?step=1&status=PAID&orderCode=${orderId}`
        );
        handleNext();
      }
    } catch (error) {
