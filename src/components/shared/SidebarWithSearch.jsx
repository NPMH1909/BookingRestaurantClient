...
import { setEmployeeTab } from "../../features/slices/employeeTabSlice";

const SidebarWithSearch = ({ SIDEBAR_SEARCH, useEmployeeTab }) => {
  const [open, setOpen] = React.useState(0);
  const dispatch = useDispatch();
  const selectedTab = useSelector((state) =>
    useEmployeeTab
      ? state.employeeSelectedTab?.value
                    ...
                    <List className="p-0 " key={"list-" + item.title.label}>
                      {item.sublist.map((subitem) => (
                        <ListItem
                          ...
                          key={subitem.label}
                          onClick={() =>
                            dispatch(useEmployeeTab ? setEmployeeTab(subitem.label) : setSelectedTab(subitem.label))
                          }
                        >
                          <ListItemPrefix>
                            <ChevronRightIcon
              ...
              <>
                <ListItem
                  ...
                  onClick={() =>
                    dispatch(
                      useEmployeeTab
                        ? setEmployeeTab(item.sublist[0].label)
                        : setSelectedTab(item.sublist[0].label)
                    )
                  }
                  style={{
                    backgroundColor:
                      selectedTab === item.sublist[0].label ? "white" : "transparent",
                    color:
                      selectedTab === item.sublist[0].label ? "#FF333A" : "inherit",
                  }}
                >
                  <ListItemPrefix>
